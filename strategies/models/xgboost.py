...
class XGBoostStrategy(Strategy): 
    ...
    explanation = "XGBoost classifier strategy using technical indicators" 
    ...
    def _execute_train(cls, model_version: Optional[str] = None) -> dict:
        ...
        n_estimators = cls.hyperparams.get("n_estimators", 100)
        max_depth = cls.hyperparams.get("max_depth", 6)
        learning_rate = cls.hyperparams.get("learning_rate", 0.3)
    ...
    def _execute_test(cls, model_version: Optional[str] = None) -> dict:
        """test the trained XGBoost strategy with confidence thresholding"""
        conf_threshold = cls.hyperparams.get("confidence_threshold", 0.50)
        print(f"[{cls.strategy_name}] Executing backtest using XGBoost predictions (confidence_threshold={conf_threshold})...")
    ...
    def _execute_trade(cls, ticker: str = 'AAPL', target_date=None, model_version: Optional[str] = None) -> int:
        """
        End-to-end execution of a single trade decision using the live XGBoost model.
